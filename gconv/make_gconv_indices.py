...
def flatten_indices(inds):
    """
    :param inds: np.ndarray of shape (nto, nti, n_size, n_size, 3), as output by
    the functions like make_d4_p4m_indices(n_size).
    :return: np.ndarray of shape (nto, nti, n_size, n_size)
    """
    n_size = inds.shape[-2]
    nti = inds.shape[1]
    T_inds = inds[..., 0]  # shape (nto, nti, n_size, n_size)
    U_inds = inds[..., 1]  # shape (nto, nti, n_size, n_size)
    V_inds = inds[..., 2]  # shape (nto, nti, n_size, n_size)
    
    inds_flat = U_inds * n_size * nti + V_inds * nti + T_inds
    return inds_flat