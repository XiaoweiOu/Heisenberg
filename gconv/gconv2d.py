...
def gconv2d(inputs, filters, strides, padding, gconv_indices, gconv_shape_info,
    ...
    :param inputs: a tensor with (batch, height, width, in channels) axes.
    :param filters: a tensor with (ksize, ksize, in channels * in transformations, out channels) axes.
    ...
    :param data_format: the order of axes. Currently only NHWC is supported
    :param name: a name for the operation (optional)
    :return: tensor with (batch, out channels, height, width) axes.
    """

    if data_format != 'NHWC':
        raise NotImplementedError('Currently only NHWC data_format is supported. Got:' + str(data_format))

    # Transform the filters
    transformed_filters = transform_filter_2d_nhwc(w=filters, flat_indices=gconv_indices, shape_info=gconv_shape_info)
