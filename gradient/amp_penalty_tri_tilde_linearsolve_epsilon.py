...
import numpy as np
import scipy
def get_gradient_amp_penalty_tri_tilde_linearsolve_epsilon(derlogs, sample_size, eloc, num_param_amp):
    """
    ...
    S_mat = O_mat_stack.T @ O_mat_stack
    S_mat_reg = S_mat + np.eye(S_mat.shape[1])*regularizer
    cholesky = scipy.linalg.cholesky(S_mat_reg, lower = True)
    final_grads = tf.cast(scipy.linalg.cho_solve((cholesky, True), O_mat_stack.T @ R_vec_weighted_stack), tf.float32)
