...
import numpy as np
import scipy
def get_gradient_amp_penalty_tri_tilde_linearsolve(derlogs, sample_size, eloc, num_param_amp):
    """
    ...
    O_mat_weighted_stack = tf.concat([mass * tf.math.real(O_mat), tf.math.imag(O_mat)], axis = 0).numpy().astype(np.float64)
    ...
    # final_grads = tf.cast(np.linalg.solve(S_mat_reg, O_mat_weighted_stack.T @ R_vec_stack), tf.float32)
    cholesky = scipy.linalg.cholesky(S_mat_reg, lower = True)
    final_grads = tf.cast(scipy.linalg.cho_solve((cholesky, True), O_mat_weighted_stack.T @ R_vec_stack), tf.float32)
