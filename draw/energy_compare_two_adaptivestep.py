...
ax.plot((np.array(energy1_filtered[:2000])-reference_energy)/number_particle,label=label1,color=color1)
ax.plot((np.array(energy2_filtered[:2000])-reference_energy)/number_particle,label=label2,color=color2)
ax.set_xlabel('Iteration')
ax.set_ylabel(r'$\Delta$E/N')
ax.set_yscale('log')
ax.grid(which='major', axis='both')
ax.legend()
plt.savefig('./draw/energy_compare_curve_adaptivestep.pdf',bbox_inches = 'tight', pad_inches=0.1)
